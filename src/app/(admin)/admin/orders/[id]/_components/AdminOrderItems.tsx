...
export default function AdminOrderItems({ orderId, orderItems, totalAmount, refundedAmount, orderStatus, paymentStatus }: AdminOrderItemsProps) {
  const isRefundFailed = orderStatus === "CANCELLED" && paymentStatus === "PAID";

  return (
    <div className="md:col-span-2 bg-white border border-[#B6925B]/20 shadow-sm overflow-hidden flex flex-col">
        ...
        {orderItems.map((item) => (
          <div key={item.id} className={`p-6 flex items-center gap-4 ${item.isCancelled ? 'opacity-50' : ''}`}>
            ...
            <div className="flex-1">
              <p className="font-bold text-[#4A3B2C] text-sm flex items-center gap-2">
                {item.product.name}
            ...
            <div className={`font-bold text-sm ${item.isCancelled ? 'text-gray-400 line-through' : 'text-[#B6925B]'}`}>
