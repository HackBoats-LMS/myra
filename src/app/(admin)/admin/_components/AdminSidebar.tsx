      ...
      <nav className="flex-1 min-h-0 p-4 mt-4 overflow-y-auto no-scrollbar">
        {sections.map((section) => (
          <div key={section.title} className="mb-5">
            <p className="px-4 mb-1.5 text-[9px] font-bold uppercase tracking-[0.2em] text-[#7A0B2E]">{section.title}</p>
              ...
              {section.links.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={onNavigate}
                    className="flex items-center gap-3 px-4 py-2.5 rounded-none hover:bg-[#7A0B2E] transition-colors text-xs font-bold uppercase tracking-widest"
                  >
