...
export default function OrderHeader({ orderId, orderItems, createdAt, status, canChangeAddress, savedAddresses }: OrderHeaderProps) {
  ...
  return (
    <div className="space-y-4 border-b border-[#7A0B2E]/20 pb-6">
      <Link
        href="/account/orders"
        className="inline-flex items-center gap-1.5 text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-[#7A0B2E] hover:text-[#2D1F2F] transition-colors"
      >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif text-[#2D1F2F] tracking-wide">
              Order Details
            </h1>
            <span className="font-mono text-xs font-bold text-[#2D1F2F] bg-white border border-[#7A0B2E]/30 px-2.5 py-1">
              #{shortId}
            </span>
          </div>
          <p className="text-[10px] sm:text-[11px] text-gray-500 uppercase tracking-widest font-bold mt-1.5">
            Placed on{" "}
            <span className="text-[#2D1F2F]">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-2 md:pt-0">
          ...
          {status === "PENDING" && (
            <CancelOrderButton orderId={orderId} orderItems={orderItems as any} />
          )}
        </div>
      </div>
    </div>
