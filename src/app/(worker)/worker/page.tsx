...
export default async function WorkerDashboardPage() {
  ...
  const inventoryStats: StatItem[] = [
    { label: "Products", value: productCount, icon: Archive, href: "/worker/products", color: "text-[#7A0B2E]" },
    { label: "Low Stock", value: lowStockCount, icon: AlertTriangle, href: "/worker/products", color: "text-red-600" },
    { label: "Collections", value: collectionCount, icon: FolderOpen, href: "/worker/collections", color: "text-[#7A0B2E]" },
  ];

  const shippingStats: StatItem[] = [
    { label: "To Ship", value: pendingOrders, icon: Truck, href: "/worker/orders", color: "text-[#7A0B2E]" },
    { label: "In Transit", value: shippedOrders, icon: Clock, href: "/worker/orders", color: "text-[#7A0B2E]" },
    { label: "Out for Delivery", value: outForDelivery, icon: MapPin, href: "/worker/orders", color: "text-[#7A0B2E]" },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="border-b border-[#7A0B2E]/20 pb-4">
        <h2 className="text-3xl font-serif font-bold text-[#2D1F2F] tracking-wide">Welcome back, {name}</h2>
          ...
          {stats.map((stat) => {
            ...
            return (
              <Link
                key={stat.label}
                href={stat.href}
                className="bg-white p-6 border border-[#7A0B2E]/20 shadow-sm flex items-center justify-between rounded-none hover:border-[#7A0B2E]/50 transition-colors"
              >
                <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {canInventory && (
          <Link href="/worker/products" className="bg-white border border-[#7A0B2E]/20 shadow-sm p-6 space-y-2 rounded-none hover:border-[#7A0B2E]/50 transition-colors">
            <div className="flex items-center gap-3">
              <Archive className="w-6 h-6 text-[#7A0B2E]" />
              <h3 className="font-serif text-lg text-[#2D1F2F]">Inventory Management</h3>
            </div>
            <p className="text-xs text-gray-500">Manage products, stock quantities, variants, and collections.</p>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7A0B2E] mt-2">Open Inventory &rarr;</p>
          </Link>
        )}
        {canShipping && (
          <Link href="/worker/orders" className="bg-white border border-[#7A0B2E]/20 shadow-sm p-6 space-y-2 rounded-none hover:border-[#7A0B2E]/50 transition-colors">
            <div className="flex items-center gap-3">
              <Truck className="w-6 h-6 text-[#7A0B2E]" />
              <h3 className="font-serif text-lg text-[#2D1F2F]">Shipping Management</h3>
            </div>
            <p className="text-xs text-gray-500">Review orders and create Shiprocket shipments with AWB tracking.</p>
            <p className="text-[10px] font-bold uppercase tracking-widest text-[#7A0B2E] mt-2">Open Shipping &rarr;</p>
          </Link>
        )}
      </div>
    </div>
