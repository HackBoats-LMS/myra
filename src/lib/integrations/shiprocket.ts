...
export function buildAdhocPayload(order: NonNullable<OrderForShipment>): AdhocOrderPayload {
  ...
  return {
    ...
    // Shiprocket computes the grand total (and for COD the amount collected at
    // delivery) as sub_total + shipping_charges. To ensure that equals the order
    // total the customer is actually charged (which includes tax and any coupon
    // discount), sub_total carries the non-shipping remainder so the sum matches
    // order.totalAmount exactly.
    shipping_charges: Math.round(Number(order.shippingAmount || 0)),
    sub_total: Math.max(
      Math.round(Number(order.totalAmount || 0) - Number(order.shippingAmount || 0)),
      0
    ),
    length: dim,
    breadth: dim,
