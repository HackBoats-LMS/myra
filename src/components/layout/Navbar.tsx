...
export default function Navbar({ cartCount, wishlistCount, isLoggedIn, navLinks = NAV_LINKS }: NavbarProps) {
  return (
    <nav className="w-full bg-white border-b border-[#B6925B]/20 flex items-center px-4 sm:px-6 md:px-8 py-3 sm:py-4 md:py-5 lg:py-3 relative z-50">
      {/* Logo (left) */}
      <div className="flex-1 flex items-center justify-start">
        <Link href="/" className="flex items-center">
          <Image
            ...
            height={60}
            priority
            className="object-contain py-1 px-2 md:py-2 md:px-4 h-10 sm:h-12 md:h-14 lg:h-16 xl:h-[72px] 2xl:h-[76px] w-auto transition-all"
          />
        </Link>
      </div>
      ...
      <div className="flex-1 flex items-center justify-end gap-3.5 sm:gap-5 md:gap-6 lg:gap-8">
        ...
        >
          ...
          <span className="text-[11px] sm:text-[12px] md:text-[13px] lg:text-[11px] font-serif lowercase tracking-normal text-[#171717] group-hover:text-[#B6925B] leading-none mt-0.5 lg:mt-0">account</span>
        </Link>
